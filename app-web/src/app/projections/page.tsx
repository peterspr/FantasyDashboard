...
export default function ProjectionsPage() {
  ...
  const columns: ColumnDef<ProjectionItem>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Projection',
        cell: ({ getValue }) => (
          <span className="font-semibold text-blue-600">{formatNumber(getValue() as number)}</span>
        ),
      },
      {
        accessorKey: 'low',
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const getPositionStats = () => {
            switch (position) {
              ...
              case 'DST':
                return (
                  <>
                    {comp.points_allowed_proj && (
                      <div>PA: {formatNumber(comp.points_allowed_proj)}</div>
                    )}
                    {comp.sacks_proj && <div>Sacks: {formatNumber(comp.sacks_proj)}</div>}
                    {comp.interceptions_proj && (
                      <div>INT: {formatNumber(comp.interceptions_proj)}</div>
                    )}
                  </>
                )
              default:
          ...
          return <div className="text-xs space-y-1">{getPositionStats()}</div>
        },
      },
    ],
    []
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Weekly Projections</h1>
      ...
      <div className="space-y-6">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            <div className="text-red-800">Error loading projections: {error.message}</div>
          </div>
        )}
