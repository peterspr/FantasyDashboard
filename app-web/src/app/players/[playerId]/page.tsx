...
export default function PlayerDetailPage() {
  ...
  const player =
    (playersData as PlayersList)?.items?.find((p) => p.player_id === playerId) ||
    usageData?.items?.[0] ||
  ...
  const renderPositionSpecificChart = () => {
    ...
    switch (position) {
      ...
      case 'RB':
        return <RBTouchDistributionChart {...commonProps} />

      case 'WR':
        return <WRTargetQualityChart {...commonProps} />

      case 'TE':
        return <TERoleDefinitionChart {...commonProps} />

      case 'DST':
        return <DSTDefensiveImpactChart {...commonProps} />
  ...
  if (
    (!isDefensePlayer && usageLoading && !usageData) ||
    (allProjectionsLoading && !allProjectionsData)
  ) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Could not find data for player ID: {playerId}</p>
        </div>
      </div>
    )
  }
  ...
  const createCompleteProjectionData = () => {
    ...
    const projectionMap = new Map(
      (allProjectionsData?.items || []).map((item: any) => [item.week, item])
    )
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">{renderAdditionalCharts()}</div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                      ...
                      return (
                        <tr key={item.week} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {components.points_allowed_proj
                              ? components.points_allowed_proj.toFixed(1)
                              : '-'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {components.interceptions_proj
                              ? components.interceptions_proj.toFixed(1)
                              : '-'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {components.fumble_recoveries_proj
                              ? components.fumble_recoveries_proj.toFixed(1)
                              : '-'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  ...
                  : // Offensive player stats rows
                    usageData?.items.map((item) => (
                      <tr key={item.week} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {item.snap_pct !== null && item.snap_pct !== undefined
                            ? (item.snap_pct * 100).toFixed(1) + '%'
                            : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {item.route_pct !== null && item.route_pct !== undefined
                            ? (item.route_pct * 100).toFixed(1) + '%'
                            : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {item.routes !== null && item.routes !== undefined && item.routes !== 0
                            ? item.routes
                            : '-'}
                        </td>
                      </tr>
          ...
          {((isDefense &&
            (!defenseProjectionData?.items || defenseProjectionData.items.length === 0)) ||
            (!isDefense && (!usageData?.items || usageData.items.length === 0))) && (
            <div className="text-center py-8 text-gray-500">
              {isDefense
                ? 'No defense statistics available for this player'
