...
export function FantasyBreakdownChart({
  ...
  const breakdownData: FantasyBreakdownDataPoint[] = data.map(item => {
    ...
    if (position === 'DST') {
      ...
      breakdown.fumble_recoveries = (components.fumble_recoveries_proj || 0) * weights.fumble_recoveries;
  ...
  const getDataKeys = (): string[] => {
    switch (position) {
      ...
      case 'DST':
        return ['points_allowed', 'sacks', 'def_interceptions', 'fumble_recoveries', 'def_tds', 'safeties', 'blocked_kicks'];
      default:
        return ['receptions', 'rec_yards', 'rec_tds', 'rush_yards', 'rush_tds', 'fumbles'];
    }
  };
  ...
  const formatRawStat = (key: string, value: number): string => {
    ...
    if (key.includes('td') || key === 'interceptions' || key === 'sacks' || 
        key === 'def_interceptions' || key === 'fumble_recoveries' || 
        key === 'safeties' || key === 'blocked_kicks') {
    ...
    <div className="space-y-4">
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          <Tooltip
            content={({ active, payload, label }) => {
              if (active && payload && payload.length) {
                ...
                const calculatedTotal = payload.reduce((sum, entry) => sum + (entry.value as number), 0);
                        ...
                        .map((entry, index) => {
                          ...
                          return (
                            <p key={index} style={{ color: entry.color }}>
                              {getLabelText(key)}: {formatRawStat(key, rawStat)} = {formatNumber(fantasyPoints)} pts
                              {total > 0 && (
          ...
          <Legend 
            formatter={(value) => getLabelText(value)}
