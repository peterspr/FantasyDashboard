...
class ApiClient {
  ...
  private async request<T>(
    ...
    // Handle 401 responses by attempting token refresh (but only once per request)
    if (response.status === 401 && !isRetry && !endpoint.includes('/auth/')) {
      try {
        ...
        // Silently fail for auth-related errors during retry
      }
    }
  ...
  // Rest of Season
  async getROSProjections(season: number, params: ROSParams = {}): Promise<ROSList> {
    const queryString = this.buildQueryString(params)
  ...
  // Authentication
  // Note: OAuth login is handled by callback page, not API client

  async refreshToken(): Promise<AuthTokens> {
    // Use direct fetch to avoid automatic retry logic that could cause duplicate requests
