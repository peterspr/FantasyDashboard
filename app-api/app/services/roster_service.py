...
class RosterSlot:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RosterSlot":
        ...
        return cls(type=data["type"], position=data.get("position"), index=data["index"])
...
class RosterService:
    ...
    async def validate_roster_move(
        self, team: Team, player_id: str, player_position: str, target_slot: RosterSlot
    ) -> None:
        """
        ...
        if self.is_slot_occupied(current_roster, target_slot):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Target slot is already occupied"
            )
        ...
        if target_slot.type == "starter":
            is_eligible = await self.check_position_eligibility(
                target_slot.position, player_position
            )
            if not is_eligible:
                raise HTTPException(
    ...
    async def get_team_roster_slots(self, team_id: UUID) -> List[TeamRoster]:
        ...
        result = await self.session.execute(select(TeamRoster).where(TeamRoster.team_id == team_id))
    ...
    async def check_position_eligibility(self, roster_position: str, player_position: str) -> bool:
    ...
    async def get_available_slots(self, team: Team, player_position: str) -> List[RosterSlot]:
    ...
    async def add_player_to_roster(
        ...
        roster_entry = TeamRoster(
            team_id=team_id, player_id=player_id, roster_slot=roster_slot.to_dict()
        )
    ...
    async def move_player_in_roster(
        ...
        result = await self.session.execute(
            select(TeamRoster).where(
                TeamRoster.team_id == team_id, TeamRoster.player_id == player_id
            )
        )
        ...
        if not roster_entry:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Player not found on team roster"
            )
        ...
        if new_roster_slot.type == "starter":
            is_eligible = await self.check_position_eligibility(
                new_roster_slot.position, player_position
            )
            if not is_eligible:
                from fastapi import HTTPException, status
    ...
    async def remove_player_from_roster(self, team_id: UUID, player_id: str) -> None:
