...
def _validate_roster_positions(roster_positions: RosterPositions) -> None:
    ...
    if roster_positions.bench < 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Bench count must be at least 1"
        )
...
async def _get_weekly_projection(session: AsyncSession, player_id: str, scoring: str = "ppr") -> Dict[str, Any] | None:
...
async def get_team_roster(
    ...
    for roster_player in roster_players:
        ...
        projection = await _get_weekly_projection(session, roster_player.player_id, team.scoring_system)
