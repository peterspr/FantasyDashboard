...
class User(Base):
    ...
    provider_id: Mapped[str] = mapped_column(String(255), nullable=False)  # OAuth provider's user ID
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=func.now(), onupdate=func.now())
    last_login_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    
    # Relationships
    teams: Mapped[List["Team"]] = relationship("Team", back_populates="user", cascade="all, delete-orphan")
...
class Team(Base):
    ...
    roster_players: Mapped[List["TeamRoster"]] = relationship("TeamRoster", back_populates="team", cascade="all, delete-orphan")
...
class TeamRoster(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    team_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("teams.id", ondelete="CASCADE"), nullable=False)
    player_id: Mapped[str] = mapped_column(String(255), nullable=False)  # References existing player data
    roster_slot: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False)  # Dynamic slot definition
    ...
    __table_args__ = (
        UniqueConstraint('team_id', 'roster_slot', name='uq_team_rosters_slot'),
    )
...
class PositionEligibility(Base):
    ...
    roster_position: Mapped[str] = mapped_column(String(10), primary_key=True)  # QB, RB, WR, TE, K, DST, FLEX, etc.
    player_position: Mapped[str] = mapped_column(String(10), primary_key=True)  # QB, RB, WR, TE, K, DST